...
- `itemOps`: A suite of safe, idempotent helper functions to prevent race conditions or data duplication when extensions run multiple times. Includes:
  ...
  - `itemOps.compareTitles(scannedTitle, dbTitle, scanDesc?, dbDesc?, category?)`: Evaluates fuzzy text identity (hardware-aware, numbers strict) and returns a Jaccard similarity object.
