<script lang="ts">
    ...
    let snapshotModal: HTMLDialogElement;
        ...
        <div class="inline-flex gap-2 items-center shrink-0 pt-1 print:hidden">
            ...
            <div class="dropdown dropdown-end">
                ...
                <ul tabindex="-1" role="menu" class="dropdown-content z-[100] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 border border-base-200 mt-2 gap-1">
                    ...
                    {/if}
                    -->
                {#if data.item?.snapshots?.length > 0}
                    <li>
                        <button type="button" class="font-medium text-base-content hover:text-primary justify-between" on:click={() => snapshotModal.showModal()}>
                            <span class="flex items-center gap-2"><i class="bi bi-clock-history text-lg opacity-70"></i> Version History</span>
                            <span class="badge badge-sm badge-ghost">{data.item.snapshots.length}</span>
                        </button>
                    </li>
                {/if}
                    {#if !data.item?.attributes?.some((a) => a.isAutoGenerated) && $page.data.role !== 'VIEWER'}
                        <li>
                    ...
                    {#if data.pluginActions?.length > 0}
                        ...
                        {#each data.pluginActions as action}
                            <li>
                                ...
                                {:else}
                                    <form method="POST" action="?/triggerPluginAction" use:enhance={() => { return async ({ update }) => { notify('info', 'Extension task started...'); await update({reset: false}); }; }} class="!p-0 block">
                                        <input type="hidden" name="actionId" value={action.id}>
                                        <button type="submit" class="font-medium text-base-content hover:text-primary flex items-start gap-2.5 w-full text-left bg-transparent border-none outline-none px-4 py-2">
                                            {#if action.icon}<i class="bi {action.icon} text-lg leading-none opacity-70 mt-0.5 shrink-0"></i>{/if}
                                            <span class="leading-tight whitespace-normal text-left">{action.label}</span>
                                        </button>
                                    </form>
...
<dialog bind:this={snapshotModal} class="modal modal-bottom sm:modal-middle backdrop-blur-sm">
    <div class="modal-box p-6 bg-base-100 shadow-2xl border border-base-200 sm:rounded-[2.5rem]">
        <h3 class="font-bold text-xl mb-4 flex items-center gap-2"><i class="bi bi-clock-history text-primary"></i> Version History</h3>
        <div class="flex flex-col gap-2">
            {#each data.item?.snapshots || [] as snap}
                <div class="flex items-center justify-between p-3 bg-base-200/50 rounded-xl border border-base-300">
                    <div>
                        <div class="font-bold text-sm">{snap.source}</div>
                        <div class="text-xs text-gray-500"><RelativeDate date={snap.createdAt} /></div>
                    </div>
                    <form method="POST" action="?/rollback" use:enhance={() => { return async ({ update }) => { snapshotModal.close(); notify('success', 'Rolled back item.'); await update({ reset: false }); }; }}>
                        <input type="hidden" name="snapshotId" value={snap.id} />
                        <button type="submit" class="btn btn-sm btn-ghost hover:bg-warning/20 hover:text-warning"><i class="bi bi-arrow-counterclockwise"></i> Restore</button>
                    </form>
                </div>
            {/each}
        </div>
        <div class="modal-action">
            <button class="btn btn-ghost rounded-xl" on:click={() => snapshotModal.close()}>Close</button>
        </div>
    </div>
    <form method="dialog" class="modal-backdrop"><button>close</button></form>
</dialog>
