...
export const actions = {
    ...
    reanalyze: async ({ locals, params }) => {
        ...
        // Wipe auto-generated attributes to allow the AI to start fresh
        await db.kVP.deleteMany({ where: { itemId: item.id, isAutoGenerated: true } });
        // Wipe LLM analysis on photos to force a re-run
        await db.photo.updateMany({ where: { itemId: item.id }, data: { llmAnalysis: null } });
