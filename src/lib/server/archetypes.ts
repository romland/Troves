...
export const BASE_ARCHETYPES = [
	{ 
		...
		contentsHint: "Tools, cables, components, gear"
	},
	{ 
		...
		contentsHint: "Clothes, shoes, accessories"
	},
	{ 
		...
		contentsHint: "Books, movies, music, games"
	},
	{ 
		...
		contentsHint: "Wine, pantry, supplies"
	},
	{ 
		...
		contentsHint: "Stamps, coins, cards, toys"
	},
	{ 
		...
		contentsHint: "Plants, rocks, shells"
	},
	{ 
        ...
        promptGuidance: "",
		askTrovesExample: "What kind of batteries does this take?",
		contentsHint: "A mix of everything"
	}
];

export function getAllArchetypes() {
    // Dynamically merge core archetypes with any registered by plugins
    return [...BASE_ARCHETYPES, ...extensionManager.getArchetypes()];
}

export function getArchetype(id: string) {
    const coreMatch = BASE_ARCHETYPES.find(a => a.id === id);
    if (coreMatch) return coreMatch;
    
    return extensionManager.getArchetype(id) || BASE_ARCHETYPES.find(a => a.id === 'generic');
}
