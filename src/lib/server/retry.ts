...
export async function withRetry<T>(
    ...
    const targetItemId = context?.itemId || (context?.targetType === 'item' ? (context.targetId || null) : null);
        ...
        try {
            ...
            if (context?.prompt) {
                ...
                if (dev) {
                    ...
                    await logActivity(targetItemId, 'LLM Debug', `Dev Mode: Prompt & Response for ${taskName}`, 'info', payload);
                }
            }
            ...
            if (attempt >= maxRetries) {
                ...
                await logActivity(targetItemId, 'LLM Error', `${taskName} failed after ${maxRetries} attempts.`, 'error');
                throw error;
            }
