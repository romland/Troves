        ...
        <div class="form-control relative w-full">
            ...
            <div class="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-0.5 overflow-hidden">
                ...
                <div class="transition-all duration-300 origin-right {q.length === 0 ? 'scale-90 opacity-0 pointer-events-none w-0' : 'scale-100 opacity-100 w-auto'}">
                    {#if !isProcessingAudio}
                        <button type="button" style="width: 1.75rem;" class="btn btn-xs btn-ghost btn-circle text-gray-400 hover:text-base-content" on:mousedown|preventDefault on:click|preventDefault={() => { q = ''; query(new Event('input'), ''); (document.querySelector('input[name="q"]') as HTMLInputElement)?.focus(); }} aria-label="Clear">
